Feature: A repository using Conventional Commits can by default use the commits in a consecutive order to calculate the next semantic version.


Scenario Outline:
    Given I clone the repository "<repository>" and checkout the commit "<checkout_commit>".
    When I call conventional-commits-next-version with the from commit "<from_commit>" and version "<current_version>".
